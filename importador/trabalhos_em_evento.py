...
for curriculo in curriculos:
  ...
  try:
    ...
    if producao_bibliografica != None:
      ...
      trabalhos_em_eventos_grupo = producao_bibliografica.find('TRABALHOS-EM-EVENTOS')

      if trabalhos_em_eventos_grupo != None:

        trabalho_em_eventos = trabalhos_em_eventos_grupo.findall('TRABALHO-EM-EVENTOS')

        for trabalho_em_evento in trabalho_em_eventos:

          dados_basicos_do_trabalho = trabalho_em_evento.find('DADOS-BASICOS-DO-TRABALHO')
          detalhamento_do_trabalho  = trabalho_em_evento.find('DETALHAMENTO-DO-TRABALHO')
          palavras_chave            = trabalho_em_evento.find('PALAVRAS-CHAVE')
          autores                   = trabalho_em_evento.findall('AUTORES')
          areas_do_conhecimento     = trabalho_em_evento.find('AREAS-DO-CONHECIMENTO')
          setores_de_atividade      = trabalho_em_evento.find('SETORES-DE-ATIVIDADE')

          if dados_basicos_do_trabalho != None:

            natureza           = texto.normaliza(dados_basicos_do_trabalho.get('NATUREZA') or '')
            titulo_do_trabalho = texto.normaliza(dados_basicos_do_trabalho.get('TITULO-DO-TRABALHO') or '')
            ano_do_trabalho    = texto.normaliza(dados_basicos_do_trabalho.get('ANO-DO-TRABALHO') or '')
            pais_do_evento     = texto.normaliza(dados_basicos_do_trabalho.get('PAIS-DO-EVENTO') or '')
            idioma             = texto.normaliza(dados_basicos_do_trabalho.get('IDIOMA') or '')
            meio_de_divulgacao = texto.normaliza(dados_basicos_do_trabalho.get('MEIO-DE-DIVULGACAO') or '')

          if detalhamento_do_trabalho != None:

            classificacao_do_evento         = texto.normaliza(detalhamento_do_trabalho.get('CLASSIFICACAO-DO-EVENTO') or '')
            nome_do_evento                  = texto.normaliza(detalhamento_do_trabalho.get('NOME-DO-EVENTO') or '')
            cidade_do_evento                = texto.normaliza(detalhamento_do_trabalho.get('CIDADE-DO-EVENTO') or '')
            ano_de_realizacao               = texto.normaliza(detalhamento_do_trabalho.get('ANO-DE-REALIZACAO') or '')
            titulo_dos_anais_ou_proceedings = texto.normaliza(detalhamento_do_trabalho.get('TITULO-DOS-ANAIS-OU-PROCEEDINGS') or '')
            volume                          = texto.normaliza(detalhamento_do_trabalho.get('VOLUME') or '')
            fasciculo                       = texto.normaliza(detalhamento_do_trabalho.get('FASCICULO') or '')
            serie                           = texto.normaliza(detalhamento_do_trabalho.get('SERIE') or '')
            pagina_inicial                  = texto.normaliza(detalhamento_do_trabalho.get('PAGINA-INICIAL') or '')
            pagina_final                    = texto.normaliza(detalhamento_do_trabalho.get('PAGINA-FINAL') or '')
            nome_da_editora                 = texto.normaliza(detalhamento_do_trabalho.get('NOME-DA-EDITORA') or '')
            cidade_da_editora               = texto.normaliza(detalhamento_do_trabalho.get('CIDADE-DA-EDITORA') or '')
          ...
          try:
            cursor.execute("""INSERT INTO tab_15_trabalhos_em_eventos 
                          (docente, natureza, titulo_do_trabalho, ano_do_trabalho, pais_evento, idioma, meio_de_divulgacao, classificacao_do_evento, nome_do_evento, ano_de_realizacao, titulo_dos_anais_ou_proceedings, volume, fasciculo, serie, pagina_inicial, pagina_final, nome_editora, cidade_da_editora, autores_nome_completo1, autores_nome_completo2, autores_nome_completo3, autores_nome_completo4, autores_nome_completo5, ordem_de_autoria1, ordem_de_autoria2, ordem_de_autoria3, palavra_chave1, palavra_chave2, palavra_chave3, palavra_chave4, palavra_chave5, palavra_chave6, nome_grande_area_do_conhecimento1, nome_grande_area_do_conhecimento2, nome_grande_area_do_conhecimento3, nome_da_area_conhecimento1, nome_da_area_conhecimento2, nome_da_area_conhecimento3, nome_sub_area_conhecimento1, nome_sub_area_conhecimento2, nome_sub_area_conhecimento3, nome_da_especialidade1, nome_da_especialidade2, nome_da_especialidade3, setor_da_atividade1, setor_da_atividade2, setor_da_atividade3, id_1dados_gerais) 
                          VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""", 
                          (nome_do_docente, natureza, titulo_do_trabalho, ano_do_trabalho, pais_do_evento, idioma, meio_de_divulgacao, classificacao_do_evento, nome_do_evento, ano_de_realizacao, titulo_dos_anais_ou_proceedings, volume, fasciculo, serie, pagina_inicial, pagina_final, nome_da_editora, cidade_da_editora, campo_autores[0], campo_autores[1], campo_autores[2], campo_autores[3], campo_autores[4], campo_autorias[0], campo_autorias[1], campo_autorias[2], palavra_chave_1, palavra_chave_2, palavra_chave_3, palavra_chave_4, palavra_chave_5, palavra_chave_6, nome_grande_area_do_conhecimento_1, nome_grande_area_do_conhecimento_2, nome_grande_area_do_conhecimento_3, nome_da_area_do_conhecimento_1, nome_da_area_do_conhecimento_2, nome_da_area_do_conhecimento_3, nome_da_sub_area_do_conhecimento_1, nome_da_sub_area_do_conhecimento_2, nome_da_sub_area_do_conhecimento_3, nome_da_especialidade_1, nome_da_especialidade_2, nome_da_especialidade_3, setor_de_atividade_1, setor_de_atividade_2, setor_de_atividade_3, identificacao))
            conexao.conn.commit()
            print('Trabalho em Evento inserido com sucesso!')
          except Exception as e:
            print('ERRO: Ao salvar o Trabalho em Evento')
            print(e)
