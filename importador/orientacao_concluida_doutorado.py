...
for curriculo in curriculos:
  ...
  outra_producao  = root.find('OUTRA-PRODUCAO')
  ...
  try:
    ...
    if outra_producao != None:

      orientacoes_concluidas_grupo = outra_producao.find('ORIENTACOES-CONCLUIDAS')
      
      if orientacoes_concluidas_grupo != None:

        orientacoes_concluidas_para_doutorados = orientacoes_concluidas_grupo.findall('ORIENTACOES-CONCLUIDAS-PARA-DOUTORADO')

        for relatorio_de_pesquisa in orientacoes_concluidas_para_doutorados:

          dados_basicos         = relatorio_de_pesquisa.find('DADOS-BASICOS-DE-ORIENTACOES-CONCLUIDAS-PARA-DOUTORADO')
          detalhamento          = relatorio_de_pesquisa.find('DETALHAMENTO-DE-ORIENTACOES-CONCLUIDAS-PARA-DOUTORADO')
          ...
          if detalhamento != None:
            ...
            tipo_de_orientacao           = texto.normaliza(detalhamento.get('TIPO-DE-ORIENTACAO') or '')
            nome_do_orientado            = texto.normaliza(detalhamento.get('NOME-DO-ORIENTADO') or '')
            nome_da_instituicao          = texto.normaliza(detalhamento.get('NOME-DA-INSTITUICAO') or '')
            nome_orgao                   = texto.normaliza(detalhamento.get('NOME-ORGAO') or '')
            nome_do_curso                = texto.normaliza(detalhamento.get('NOME-DO-CURSO') or '')
            flag_bolsa                   = texto.normaliza(detalhamento.get('FLAG-BOLSA') or '')
            nome_da_agencia              = texto.normaliza(detalhamento.get('NOME-DA-AGENCIA') or '')
            tipo_de_orientacao_concluida = texto.normaliza(detalhamento.get('TIPO-DE-ORIENTACAO-CONCLUIDA') or '')
