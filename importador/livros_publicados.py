  ...
  try:
    ...
    if producao_bibliografica != None:
      ...
      if livros_e_capitulos != None:
        ...
        livros_publicados_ou_organizados_grupo = livros_e_capitulos.find('LIVROS-PUBLICADOS-OU-ORGANIZADOS')

        if livros_publicados_ou_organizados_grupo != None:

          livro_publicado_ou_organizados = livros_publicados_ou_organizados_grupo.findall('LIVRO-PUBLICADO-OU-ORGANIZADO')

          for livro_publicado_ou_organizado in livro_publicado_ou_organizados:

            dados_basicos_do_livro = livro_publicado_ou_organizado.find('DADOS-BASICOS-DO-LIVRO')
            detalhamento_do_livro  = livro_publicado_ou_organizado.find('DETALHAMENTO-DO-LIVRO')
            palavras_chave         = livro_publicado_ou_organizado.find('PALAVRAS-CHAVE')
            autores                = livro_publicado_ou_organizado.findall('AUTORES')
            areas_do_conhecimento  = livro_publicado_ou_organizado.find('AREAS-DO-CONHECIMENTO')
            setores_de_atividade   = livro_publicado_ou_organizado.find('SETORES-DE-ATIVIDADE')

            if dados_basicos_do_livro != None:

              tipo               = texto.normaliza(dados_basicos_do_livro.get('TIPO') or '')
              natureza           = texto.normaliza(dados_basicos_do_livro.get('NATUREZA') or '')
              titulo_do_livro    = texto.normaliza(dados_basicos_do_livro.get('TITULO-DO-LIVRO') or '')
              ano                = texto.normaliza(dados_basicos_do_livro.get('ANO') or '')
              pais               = texto.normaliza(dados_basicos_do_livro.get('PAIS-DE-PUBLICACAO') or '')
              idioma             = texto.normaliza(dados_basicos_do_livro.get('IDIOMA') or '')
              meio_de_divulgacao = texto.normaliza(dados_basicos_do_livro.get('MEIO-DE-DIVULGACAO') or '')

            if detalhamento_do_livro != None:

              numero_de_volumes        = texto.normaliza(detalhamento_do_livro.get('NUMERO-DE-VOLUMES') or '')
              numero_de_paginas        = texto.normaliza(detalhamento_do_livro.get('NUMERO-DE-PAGINAS') or '')
              numero_da_edicao_revisao = texto.normaliza(detalhamento_do_livro.get('NUMERO-DA-EDICAO-REVISAO') or '')
              numero_da_serie          = texto.normaliza(detalhamento_do_livro.get('NUMERO-DA-SERIE') or '')
              cidade_da_editora        = texto.normaliza(detalhamento_do_livro.get('CIDADE-DA-EDITORA') or '')
              nome_da_editora          = texto.normaliza(detalhamento_do_livro.get('NOME-DA-EDITORA') or '')
