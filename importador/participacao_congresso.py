...
for curriculo in curriculos:
  ...
  try:
    ...
    if dados_complementares != None:
      ...
      participacao_em_eventos_congressos = dados_complementares.find('PARTICIPACAO-EM-EVENTOS-CONGRESSOS')
      
      if participacao_em_eventos_congressos != None:

        participacao_em_congressos = participacao_em_eventos_congressos.findall('PARTICIPACAO-EM-CONGRESSO')

        for participacao_em_congresso in participacao_em_congressos:

          dados_basicos         = participacao_em_congresso.find('DADOS-BASICOS-DA-PARTICIPACAO-EM-CONGRESSO')
          detalhamento          = participacao_em_congresso.find('DETALHAMENTO-DA-PARTICIPACAO-EM-CONGRESSO')
          palavras_chave        = participacao_em_congresso.find('PALAVRAS-CHAVE')
          participantes         = participacao_em_congresso.findall('PARTICIPANTE-DE-EVENTOS-CONGRESSOS')
          areas_do_conhecimento = participacao_em_congresso.find('AREAS-DO-CONHECIMENTO')
          setores_de_atividade  = participacao_em_congresso.find('SETORES-DE-ATIVIDADE')
          ...
          if detalhamento != None:
            ...
            nome_do_evento   = texto.normaliza(detalhamento.get('NOME-DO-EVENTO') or '')
            local_do_evento  = texto.normaliza(detalhamento.get('LOCAL-DO-EVENTO') or '')
            cidade_do_evento = texto.normaliza(detalhamento.get('CIDADE-DO-EVENTO') or '')

          nomes_participantes = []

          for participante in participantes:
            
            nome_completo_do_participante = texto.normaliza(participante.get('NOME-COMPLETO-DO-PARTICIPANTE-DE-EVENTOS-CONGRESSO') or '')
          ...
          try:
            ...
            conexao.conn.commit()
            print('Participacao em congresso inserida com sucesso!')
          except Exception as e:
            print('ERRO: Ao salvar o Participacao em congresso')
            print(e)
